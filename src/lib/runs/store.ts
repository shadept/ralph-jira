...
export async function ensureRunArtifacts(projectPath: string) {
  await Promise.all([ensureDir(runsDir(projectPath)), ensureDir(sandboxRoot(projectPath))]);
}

export async function readRun(projectPath: string, runId: string): Promise<RunRecord> {
  const buffer = await fs.readFile(runFilePath(projectPath, runId), 'utf-8');
  return RunRecordSchema.parse(JSON.parse(buffer));
}

export async function writeRun(projectPath: string, run: RunRecord): Promise<void> {
...
export async function listRuns(projectPath: string): Promise<RunRecord[]> {
  ...
  for (const file of runFiles) {
    try {
      const buffer = await fs.readFile(path.join(runsDir(projectPath), file), 'utf-8');
...
export async function tailLog(filePath: string, maxLines = 100): Promise<string[]> {
  try {
...
export function boardSourcePathFromId(boardId: string) {
  if (boardId === 'prd' || boardId === 'active' || boardId === 'initial-sprint') {
...
export async function upsertRun(projectPath: string, run: RunRecord, patch: Partial<RunRecord>) {
