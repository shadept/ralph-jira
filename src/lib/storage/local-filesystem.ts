...
import { Board, ProjectSettings, RunLog, BoardSchema, ProjectSettingsSchema, RunLogSchema } from '../schemas';
...
export class LocalFilesystemAdapter implements StorageAdapter {
  ...
  async writeBoard(board: Board): Promise<void> {
    ...
    const filePath = board.id === 'active' || board.id === 'prd' || board.id === 'initial-sprint'
      ? path.join(this.plansDir, 'prd.json')
  ...
  async listBoards(): Promise<string[]> {
    ...
    const boardFiles = files.filter(f => f.endsWith('.json') && f !== 'settings.json');
  ...
  async appendProgress(entry: string): Promise<void> {
    ...
    try {
      ...
      await fs.writeFile(this.progressFile, `# Project Progress Log${logEntry}`, 'utf-8');
    }
  }
