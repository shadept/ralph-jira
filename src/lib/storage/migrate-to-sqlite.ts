...
async function migrateToSqlite(projectPath: string): Promise<MigrationResult> {
  ...
  try {
    ...
    try {
      ...
      console.error(`Failed to list boards: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
      ...
      console.error(`Failed to list run logs: ${error instanceof Error ? error.message : String(error)}`);
    }
...
function printSummary(result: MigrationResult): void {
  console.log('\n========== Migration Summary ==========');
  console.log(`Boards: ${result.boards.migrated} migrated, ${result.boards.errors.length} errors`);
  console.log(`Settings: ${result.settings.migrated ? 'migrated' : 'failed'}`);
  console.log(`Run Logs: ${result.runLogs.migrated} migrated, ${result.runLogs.errors.length} errors`);
