...
export class WorkspaceManager {
  ...
  async autoSaveUncommittedChanges() {
    ...
    // Stage all changes
    const addResult = await this.runCommand('git', ['add', '-A'], this.sandboxDir);
    if (addResult.code !== 0) {
      await this.appendLog(`Failed to stage changes: ${addResult.stderr || addResult.stdout}\n`);
      return false;
    }
    ...
    if (commitResult.code !== 0) {
      await this.appendLog(`Failed to auto-save commit: ${commitResult.stderr || commitResult.stdout}\n`);
      return false;
    }
  ...
  async pushBranch() {
    ...
    await this.appendLog(`Successfully pushed branch ${branchName} to origin\n`);
    return true;
  }
  ...
  async createPullRequest() {
    ...
    try {
      ...
      if (ghCheck.code !== 0) {
        await this.appendLog('GitHub CLI (gh) not available (exit code != 0). Skipping PR creation.\n');
        return null;
      }
      ...
      if (authCheck.code !== 0) {
        await this.appendLog('GitHub CLI not authenticated. Skipping PR creation.\n');
        return null;
      }
    } catch (err) {
      await this.appendLog(`GitHub CLI check failed: ${err.message}. Skipping PR creation.\n`);
      return null;
    }
  ...
  async runSetup(setupCommands, checkCancellation) {
    if (!setupCommands.length) {
      await this.appendLog('No setup commands configured. Skipping setup step.\n');
      return;
    }
   ...
   * @returns {Promise<{prUrl: string|null, cleanedUp: boolean}>}
   */
  async finalize() {
    const branchName = this.getEffectiveBranchName();
    let prUrl = null;
    ...
    try {
      ...
      await this.appendLog(`Warning: Could not auto-save uncommitted changes: ${saveError.message}\n`);
    }
    ...
    if (commitCount === 0) {
      ...
      await this.appendLog(`Branch ${branchName} has no commits. Cleaning up.\n`);
      try {
        await this.removeWorktree({ silent: false });
      ...
      if (pushSuccess) {
        ...
        await this.appendLog(`Push failed. Leaving worktree at ${this.sandboxDir} for human review.\n`);
        await this.appendLog(`To clean up manually: git worktree remove ${this.sandboxDir}\n`);
      }
    } else {
      // Error determining commit count - leave worktree for manual review
      await this.appendLog(`Could not determine commit status for ${branchName}. Leaving worktree in place.\n`);
    }

    return { prUrl, cleanedUp };
  }
}
