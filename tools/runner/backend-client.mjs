...
export class BackendClient {
	...
	async readRun(runId) {
		...
		if (!response.ok) {
			...
			throw new Error(
				`Failed to read run ${runId}: ${response.status} - ${text}`,
			);
		}
		const data = await response.json();
		console.log(
			"[backend-client] readRun success, run status:",
			data.run?.status,
		);
		return data.run;
	}
	...
	async readSettings() {
		...
		const response = await fetch(url, { headers: this.#getHeaders() });
		console.log(
			"[backend-client] readSettings response status:",
			response.status,
		);
		if (!response.ok) {
			const text = await response.text().catch(() => "(no body)");
			console.error(
				"[backend-client] readSettings failed:",
				response.status,
				text,
			);
	...
	async readSprint(sprintId) {
		...
		const response = await fetch(url, { headers: this.#getHeaders() });
		console.log(
			"[backend-client] readSprint response status:",
			response.status,
		);
		if (!response.ok) {
			const text = await response.text().catch(() => "(no body)");
			console.error(
				"[backend-client] readSprint failed:",
				response.status,
				text,
			);
			throw new Error(
				`Failed to read sprint ${sprintId}: ${response.status} - ${text}`,
			);
		}
		const data = await response.json();
		console.log(
			"[backend-client] readSprint success, tasks count:",
			data?.sprint?.tasks?.length || 0,
		);
		return data.sprint;
	}
	...
	async writeSprint(sprint) {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to write sprint ${sprint.id}: ${response.status}`,
			);
		}
	}
	...
	async checkCancellation(runId) {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to check cancellation for ${runId}: ${response.status}`,
			);
		}
	...
	async createCommand(runId, commandData) {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to create command for ${runId}: ${response.status}`,
			);
		}
		const data = await response.json();
		return data.command;
	...
	async updateCommand(runId, commandData) {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to update command for ${runId}: ${response.status}`,
			);
		}
		const data = await response.json();
		return data.command;
...
export function initBackendClient(options) {
	const authToken =
		options.authToken || process.env.RUN_LOOP_AUTH_TOKEN || null;
	...
	if (!options.baseUrl) {
		throw new Error(
			"BackendClient requires baseUrl (set RUN_LOOP_API_URL environment variable)",
		);
	}
	if (!options.projectId) {
		throw new Error(
			"BackendClient requires projectId (set RUN_LOOP_PROJECT_ID environment variable)",
		);
	}
...
export function getBackendClient() {
	if (!_backendClient) {
		throw new Error(
			"BackendClient not initialized. Call initBackendClient() first.",
		);
	}
	return _backendClient;
}
